 ...
 * `$ref` the emitter left relative, writes `spec_objects_enterprise/schemas/`
...
const NORMALIZATION = {
  ...
  version: "1.0.0",
  issue: "https://github.com/agent-ix/spec-objects-enterprise/issues/4",
};

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const sourceDir = resolve(repoRoot, "typespec");
const packageDir = resolve(repoRoot, "spec_objects_enterprise");
...
function moduleBase() {
  ...
  const expected = `https://schemas.agent-ix.org/agent-ix/spec-objects-enterprise/${version}/`;
...
function emit() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), "spec-objects-enterprise-emit-"));
  try {
    compile(scratch);
