...
- `spec_objects_enterprise/schemas/toolchain.json`: compiler and emitter names
...
- If any emitted file differs from the committed output, a committed file under `spec_objects_enterprise/schemas/` is stale (it has no emitted counterpart in this run), `toolchain.json` differs, or a manifest digest differs from the shipped bytes, then the check SHALL exit non-zero naming each such file.
