    ...
    try:
        ...
        if install.returncode == 0:
            ...
            manifest_path = os.path.expanduser(
                "~/.ix/filament/modules/spec-objects-enterprise/semantic/package-manifest.json"
            )
        ...
        quoin("module", "install", "spec-objects-enterprise")
...
def test_quoin_module_install_succeeds_and_lists_the_module():
    ...
    assert "spec-objects-enterprise" in observed["listing"].stdout
    assert (
...
def test_the_install_records_the_prior_listing_installs_and_lists_the_module():
    ...
    assert "semantic." not in combined or "error" not in combined.lower(), combined
    assert "spec-objects-enterprise" in observed["listing"].stdout  # step 3
...
def test_the_roundtrip_derives_the_package_manifest_and_restores_state():
    ...
    assert derived["package"]["identity"] == "agent-ix/spec-objects-enterprise"
