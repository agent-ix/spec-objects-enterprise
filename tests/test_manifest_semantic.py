...
`data_schema`, locator preservation, the relation-verb vocabulary, and what
Quire's loader refuses.
"""
...
def test_the_semantic_block_carries_the_nine_admitted_keys_and_seven_exports(
    semantic_block,
):
    ...
    # The advisory-only-until-promotion merge gate is carried by these two keys.
...
def test_allowed_links_equals_the_emitted_relation_verb_enum():
    """The manifest's edge vocabulary and the schema's relation vocabulary
    cannot drift apart: the schema is the authority and the manifest follows."""
    for name in OBJECT_TYPES:
        allowed = set(object_type(name)["allowed_links"])
        declared = set(schema_json(VERB_ENUM_OF[name])["enum"])
        assert allowed == declared, (name, allowed ^ declared)
