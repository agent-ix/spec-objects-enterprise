...
- [x] **TC-001**: the 0.2.0 manifest validates against the vendored FR-035 module-manifest schema through `quire.validate_manifest`.
- [x] **TC-002..TC-004**: activation, idempotent re-activation and registry read-back; `🚧`, needs a running filament-core.
- [x] **TC-005, TC-006**: the two StR demonstrations; `🚧`.
- [x] **TC-075**: StR-001-VC-3 — every object type ships a typed schema a fixture reader can consume, and a capability, a KPI definition and an observation-shaped record are distinguishable by schema alone.
- [x] **Trace tags on every test**: `@pytest.mark.trace("TC-0NN", "FR-0NN-AC-N")` on the test function, in the `pytest-trace-marker` form the engine declares.
- [x] **Trace-tag audit** before the PR: run `black` and `ruff` first, then confirm no marker was wrapped onto a line the form cannot read, no TC id sits on a module or class docstring or a plain helper, and no bare TC id appears in a comment where it would bind the next symbol.
- [x] **`quire coverage --scope .`** reconciled against the `spec/tests.md` figure, with the two numbers' meanings stated.
