...
- [ ] **Emit and commit** `spec_objects_enterprise/schemas/*.json` plus `toolchain.json`.
- [ ] **Assert the emitted set** equals what `toolchain.json` lists — the seven object-type models plus the support models FR-004 Outputs names — with compiler and emitter 1.15.0 recorded.
- [ ] **Assert every `$id`** is `<base><Model>.json` with the version segment read from `manifest.yaml`, never hard-coded, and every `$schema` is 2020-12.
- [ ] **Assert every `$ref`** resolves to a shipped sibling or to semantic-core 0.1.0, and to no other host or version.
- [ ] **Wheel payload**: `make build` produces a wheel containing every emitted schema.
- [ ] **npm payload**: `npm pack` produces a tarball with `manifest.yaml` and a sibling `schemas/<Model>.json` for every exported type's `data_schema.schema` path, and the staged copies are removed from the repository root afterwards.
- [ ] **Atomic bump**: bumping the manifest `version` and the `@jsonSchema` base together re-emits every `$id`, `$ref` and digest at the new version and `make schemas-check` exits zero; bumping one half exits non-zero.
